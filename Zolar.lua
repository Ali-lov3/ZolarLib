...
if GlobalEnv.Zolar and GlobalEnv.Zolar.Unload then
 GlobalEnv.Zolar:Unload()
end
...
local Library = { } do
 ...
 local FitX = (Viewport.X * 0.9) / Library.WindowWidth
 local FitY = (Viewport.Y * 0.86) / Library.WindowHeight
 Scale = Scale * math.clamp(math.min(FitX, FitY), 0.3, 0.86)
 end
 ...
 local function IsLiveGui(Object)
 if not Object then return false end
 if not Object:IsDescendantOf(Library.Holder.Instance)
 and not Object:IsDescendantOf(Library.PopupHolder.Instance) then
 return false
 end
 ...
 if Current == Library.Holder.Instance or Current == Library.PopupHolder.Instance then
 return true
 end
 ...
 if not Object or not Object.Visible or not IsLiveGui(Object) then continue end
 ...
 local Position = Input.Position
 if PointInside(Position, Frame.Instance) then return end
 if PointInside(Position, GetAnchor()) then return end
 ...
 if IsMobile then
 local MobileToggle = MakeFrame({
 Parent = Library.Holder.Instance,
 Anchor = Vector2.new(0, 0),
 Pos = UDim2.new(0, 18, 1, -76),
 ...
 MakeShadow(MobileToggle.Instance, Color3.new(0, 0, 0), 0, 8, 0.35)
 ...
 local MobileToggleHit = MakeButton({
 Parent = MobileToggle.Instance,
 Z = 102
 })
 ...
 local MobileStart
 local MobileOrigin
 local MobileInputConnection

 MobileToggleHit:Connect("InputBegan", function(Input)
 local IsClick = Input.UserInputType == Enum.UserInputType.MouseButton1
 local IsTouch = Input.UserInputType == Enum.UserInputType.Touch
 ...
 MobileOrigin = Vector2.new(
 MobileToggle.Instance.Position.X.Offset,
 MobileToggle.Instance.Position.Y.Offset
 )

 if MobileInputConnection then
 MobileInputConnection:Disconnect()
 end

 MobileInputConnection = Input.Changed:Connect(function()
 if Input.UserInputState ~= Enum.UserInputState.End then return end
 ...
 if not MobileDragging or (not IsMove and not IsTouch) then return end

 local Scale = Library:GetScreenScale()
 local Viewport = workspace.CurrentCamera.ViewportSize / Scale
 local Delta = (Input.Position - MobileStart) / Scale

 if Delta.Magnitude > 8 then
 MobileMoved = true
 end

 local Size = MobileToggle.Instance.AbsoluteSize / Scale
 local NewX = math.clamp(MobileOrigin.X + Delta.X, 18, math.max(Viewport.X - Size.X - 18, 18))
 local NewY = math.clamp(MobileOrigin.Y + Delta.Y, 18, math.max(Viewport.Y - Size.Y - 18, 18))

 MobileToggle.Instance.Position = UDim2.fromOffset(NewX, NewY)
 end)

 MobileToggleHit:Connect("MouseButton1Down", function()
 if MobileMoved then
 MobileMoved = false
 return
 end
